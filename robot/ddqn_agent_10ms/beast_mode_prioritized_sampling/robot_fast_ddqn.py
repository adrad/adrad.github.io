...
class MudBotClient(QtWidgets.QWidget):
	...
	def get_action_from_model(self):
		...
		try:
			...
			best_action = self.actiondict[max_val_index]
		except:
	...
	def replay(self):
		...
		#samples = random.sample(self.memory, batch_size)				rsamples, psamples = self.get_prioritized_samples(self.memory, batch_size)
