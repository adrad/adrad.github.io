...
int resist_cold(creature *ply_ptr, cmd *cmnd, int how )
{
    ...
    if(cmnd->num == 2) {
        ply_ptr->lasttime[LT_RCOLD].ltime = get_modified_time();
...
if (F_ISSET(ply_ptr->parent_rom,RPMEXT)){
        ...
        crt_ptr->lasttime[LT_RCOLD].ltime = get_modified_time();
...
int breathe_water(creature *ply_ptr, cmd *cmnd, int how )
{
    ...
    if(cmnd->num == 2) {
        ply_ptr->lasttime[LT_BRWAT].ltime = get_modified_time();
	...
	if (F_ISSET(ply_ptr->parent_rom,RPMEXT)){
        ...
        crt_ptr->lasttime[LT_BRWAT].ltime = get_modified_time();
        broadcast_rom2(fd, crt_ptr->fd, ply_ptr->rom_num, 
...
int earth_shield(creature *ply_ptr, cmd *cmnd, int how )
{
    ...
    if(cmnd->num == 2) {
        ply_ptr->lasttime[LT_SSHLD].ltime = get_modified_time();
	...
	if (F_ISSET(ply_ptr->parent_rom,RPMEXT)){
        ...
        crt_ptr->lasttime[LT_SSHLD].ltime = get_modified_time();
        broadcast_rom2(fd, crt_ptr->fd, ply_ptr->rom_num, 
