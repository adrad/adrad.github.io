...
void startup_mordor(void)
{
	...
	if(AUTOSHUTDOWN)
		if (!Shutdown.interval){
			Shutdown.ltime = get_modified_time(); //time(0);
			Shutdown.interval = 43200L;
		}


	umask(000);
	srand(getpid() + get_modified_time());//;time(0));
	load_lockouts();
	...
	StartTime = get_modified_time();
	//create st0 timeval to re-use throughout game for tracking time
