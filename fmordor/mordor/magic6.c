...
int resist_magic(creature *ply_ptr, cmd *cmnd, int how )
{
	...
	if(cmnd->num == 2) {
		ply_ptr->lasttime[LT_RMAGI].ltime = get_modified_time();
...
if (F_ISSET(ply_ptr->parent_rom,RPMEXT)){
		...
		crt_ptr->lasttime[LT_RMAGI].ltime = get_modified_time();
...
int know_alignment(creature	*ply_ptr, cmd *cmnd, int how )
{
	...
	if(cmnd->num == 2) {
		ply_ptr->lasttime[LT_KNOWA].ltime = get_modified_time();
		...
		crt_ptr->lasttime[LT_KNOWA].ltime = get_modified_time();
		broadcast_rom2(fd, crt_ptr->fd, ply_ptr->rom_num, 
