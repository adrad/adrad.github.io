...
/* handle poison  */
	   ...
	   if (F_ISSET(ply_ptr, PDISEA)){
        ...
        ply_ptr->lasttime[LT_ATTCK].ltime = get_modified_time();
		ANSI(ply_ptr->fd, AFC_BLUE);
...
int check_for_spam( creature *ply_ptr )
{
	...
	if ( ply_ptr->class < CARETAKER )
	{
		...
		{
			/* 40 in one second is spam */
			if(++ply_ptr->lasttime[LT_PSEND].misc > 39) 
			{
		...
		{
			/* 60 in 2 seconds is spam */
			if(++ply_ptr->lasttime[LT_PSEND].misc > 59) 
			{
				silence_spammer( ply_ptr );
				return(1);
			}
		...
		{
			/* 70 in 3 seconds is spam */
			if(++ply_ptr->lasttime[LT_PSEND].misc > 69 ) 
			{
				silence_spammer( ply_ptr );
				return(1);
			}
...
void silence_spammer( creature *ply_ptr )
{
	...
	if ( ply_ptr )
	{
		...
		{
			/* first spam */
			ply_ptr->lasttime[LT_SILNC].ltime = get_modified_time();
